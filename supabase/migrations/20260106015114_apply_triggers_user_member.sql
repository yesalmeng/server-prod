...
begin
    ...
    row_pk_json := case
        ...
        --     then jsonb_build_object('id', row_json->>'id')
        when TG_TABLE_SCHEMA = 'public' and TG_TABLE_NAME = 'user' 
            then jsonb_build_object('id', row_json->>'id')
        when TG_TABLE_SCHEMA = 'public' and TG_TABLE_NAME = 'user_role'
            then jsonb_build_object('id', row_json->>'id')
        when TG_TABLE_SCHEMA = 'public' and TG_TABLE_NAME = 'member'
            then jsonb_build_object('id', row_json->>'id')
        else null
    end;
...
drop trigger if exists log_audit_user on public.user;
create trigger log_audit_user
after insert or update or delete on public.user
for each row execute function audit.log_audit_trail();

drop trigger if exists log_audit_user_role on public.user_role;
create trigger log_audit_user_role
after insert or update or delete on public.user_role
for each row execute function audit.log_audit_trail();

drop trigger if exists log_audit_member on public.member;
create trigger log_audit_member
